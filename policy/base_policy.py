    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def compute_episodic_return(
        ...
        if v_s_ is None:
            ...
            v_s_ = np.array(v_s_.flatten())
            v_s_ = v_s_ * BasePolicy.value_mask(buffer, indices)
        v_s = np.roll(v_s_, 1) if v_s is None else np.array(v_s.flatten())
    ...
    def compute_nstep_return(
        ...
        target_q = np.array(target_q_torch.reshape(bsz, -1))
        target_q = target_q * BasePolicy.value_mask(buffer, terminal).reshape(-1, 1)
        end_flag = buffer.done.copy()
        end_flag[buffer.unfinished_index()] = True
        target_q = _nstep_return(rew, end_flag, target_q, indices, gamma, n_step)

        batch.returns = torch.Tensor(target_q, target_q_torch)
        if hasattr(batch, "weight"):  # prio buffer update
            batch.weight = torch.Tensor(batch.weight, target_q_torch)
        return batch
